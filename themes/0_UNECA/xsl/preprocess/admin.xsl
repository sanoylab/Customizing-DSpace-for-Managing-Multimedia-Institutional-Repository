...
<xsl:stylesheet
    ...
    <xsl:template match="*[not(@id='aspect.artifactbrowser.ConfigurableBrowse.div.browse-navigation')
      and not(@id='aspect.administrative.WithdrawnItems.div.browse-navigation')
      and not(@id='aspect.administrative.PrivateItems.div.browse-navigation')
      and not(@id='aspect.administrative.WithdrawnItems.div.browse-controls')
      and not(@id='aspect.administrative.PrivateItems.div.browse-controls')
      and not(starts-with(@n,'featured-item'))]
      /dri:p[
      count(./dri:field)>1
      and not(dri:field[position()=1 and @type='text'] and dri:field[position()=2 and @type='button'])]">
    ...
    <xsl:template match="dri:div[@id='aspect.general.NoticeTransformer.div.general-message']">
        <div>
            ...
            <xsl:attribute name="rend">
                ...
                <xsl:choose>
                    <xsl:when test="contains(@rend,'success' )">
                        <xsl:text> alert-success</xsl:text>
                    </xsl:when>
                    <xsl:when test="contains(@rend,'failure' )">
                        <xsl:text> alert-danger</xsl:text>
                    </xsl:when>
                    <xsl:when test="contains(@rend,'neutral' )">
