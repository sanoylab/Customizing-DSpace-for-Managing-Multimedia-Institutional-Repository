...
<xsl:stylesheet
    ...
    <xsl:template match="dri:referenceSet[@type = 'summaryList' and (contains(@rend,'recent-submission'))]" priority="2">
        <xsl:apply-templates select="dri:head"/>
        <!-- Here we decide whether we have a hierarchical list or a flat one -->
        <div class="row ds-artifact-list">
            <div class="col-xs-2 col-sm-1 icon-navigate-parent-empty">
            </div>
            <div class="col-xs-2 col-sm-1 icon-navigate-parent">
                <i class="fa fa-chevron-left icon-navigate left"></i>
            </div>
            ...
            <div class="col-xs-2 col-sm-1 icon-navigate-parent">
                <i class="fa fa-chevron-right icon-navigate right"></i>
            </div>
        </div>

        <xsl:if test="not(@n='site-featured-items')">
    ...
    <xsl:template match="dri:referenceSet[(contains(@rend,'recent-submission'))]/dri:reference" mode="summaryList">
        <xsl:variable name="externalMetadataURL">
            <xsl:text>cocoon:/</xsl:text>
            <xsl:value-of select="@url"/>
            <!-- Since this is a summary only grab the descriptive metadata, and the thumbnails -->
     ...
     <!--   <xsl:comment>External Metadata URL:
        ...
        <div class="col-xs-6 col-sm-3 ds-artifact-item">
            <xsl:apply-templates select="document($externalMetadataURL)" mode="summaryList-recent-submission"/>
            <xsl:apply-templates/>
        </div>
    </xsl:template>

    <xsl:template match="dri:referenceSet[@rend='related-items']/dri:reference" mode="summaryList">
        <xsl:variable name="externalMetadataURL">
            <xsl:text>cocoon:/</xsl:text>
            <xsl:value-of select="@url"/>
            <!-- Since this is a summary only grab the descriptive metadata, and the thumbnails -->
        ...
        <div class="clearfix">
            <xsl:apply-templates select="document($externalMetadataURL)" mode="related-items-list"/>
            <xsl:apply-templates/>
        </div>
    </xsl:template>
    ...
    <xsl:template match="mets:METS[mets:dmdSec/mets:mdWrap[@OTHERMDTYPE='DIM']]" mode="summaryList">
        <xsl:param name="submode" select="'normal'"/>
        <xsl:choose>
    ...
    <xsl:template match="mets:METS[mets:dmdSec/mets:mdWrap[@OTHERMDTYPE='DIM']]" mode="summaryList-recent-submission">
        <xsl:param name="submode" select="'recent-submission'"/>
        <xsl:choose>
            <xsl:when test="@LABEL='DSpace Item'">
                <xsl:call-template name="itemSummaryList-DIM">
                    <xsl:with-param name="submode" select="$submode"/>
