...
<xsl:stylesheet
    ...
    <xsl:template match="//dri:list[@id='aspect.discovery.SimpleSearch.list.search_bitstreams-list']" priority="999"/>

    <xsl:template match="//dri:list[@id='aspect.discovery.SimpleSearch.list.search_bitstreams-list']" mode="filefilter" priority="999">
        <div class="search_bitstreams_div hidden">
                <i18n:text>
                    <xsl:value-of select="//dri:field[@id='aspect.discovery.SimpleSearch.field.search_bitstreams']/dri:help"/>
                </i18n:text>

            <xsl:apply-templates select="//dri:field[@id='aspect.discovery.SimpleSearch.field.search_bitstreams']" />
        </div>
    </xsl:template>
    ...
    <xsl:template match="dri:row[@id='aspect.discovery.SimpleSearch.row.filter-controls']">
        <div>
            ...
            <xsl:apply-templates select="//dri:list[@id='aspect.discovery.SimpleSearch.list.search_bitstreams-list']" mode="filefilter"/>
    ...
    <xsl:template match="dri:div[@rend='controls-gear-wrapper' and @n='search-controls-gear']">
        ...
        <xsl:call-template name="rss-button"/>

    </xsl:template>

    <xsl:template name="rss-button">
        <a target="_blank">
            <xsl:attribute name="href">
                <xsl:value-of select="//dri:div[@id='aspect.discovery.RssSearchTransformer.div.rss-button-div']/dri:p/dri:xref/@target"/>
            </xsl:attribute>
            <i class="fa fa-rss btn btn-default rss-button"></i>
        </a>
    </xsl:template>
