...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
    ...
    <xsl:import href="multimedia.xsl"/>
    <xsl:import href="taxonomy.xsl"/>
    <xsl:import href="workflowadditions.xsl"/>
    <xsl:import href="statistics.xsl"/>
    <xsl:import href="uneca-imports.xsl"/>
    <!--<xsl:import href="../../ReportingSuite/statistics.xsl"/>-->

    <xsl:import href="core/static.xsl"/>
    <xsl:output indent="yes"/>

    <xsl:template match="//dri:div[@n='FeaturedItemsListTransformer']">
        <!-- suppressing the featured item list at its natural location-->
        <!-- the new location it is rendered at is defined in aspect/artifactbrowser/community-view.xsl -->
    </xsl:template>
